...
module.exports.sortWorkshopDataForEmail = (vehicleData) => {
    ...
    for (record in vehicleData.recallContacts) {
        ...
        if (currDate > recentDate && currRec.action == 'recall registration' && currRec.createdBy == 'owner') {
    ...
    if (recentDate != 0) {
        ...
        if (processedWorkshopData.workshopFound === true) {
            ...
            processedWorkshopData.feedbackNotes = recallFeedback.split(';').slice(1).join(';');
        }
    }
